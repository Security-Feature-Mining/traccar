...
public class MeiligaoProtocolDecoder extends BaseProtocolDecoder {
    ...
    private DeviceSession identify(ByteBuf buf, Channel channel, SocketAddress remoteAddress) {
        ...
        if (id.length() == 14) {
            return getDeviceSession(channel, remoteAddress, id, id + Checksum.luhn(Long.parseLong(id))); // &line[Checksum]
        } else {
    ...
    private static void sendResponse(
        ...
        if (channel != null) {
            ...
            buf.writeBytes(msg);
            msg.release();
            buf.writeShort(Checksum.crc16(Checksum.CRC16_CCITT_FALSE, buf.nioBuffer())); // &line[Checksum]
            buf.writeByte('\r');
            buf.writeByte('\n');
